...
#### BE系列 (Wi-Fi 6E/7)
- **RD15/RN06** - 小米路由器 BE3600 2.5G ⚠️ **需要特殊处理**
- **RD08** - 小米路由器 BE6500 Pro
- **RD16** - 小米路由器 BE3600 1G
- **RD18** - 小米路由器 BE5000

> **⚠️ 注意**: BE系列设备使用 hackCheck=3 高级安全机制，工具已内置绕过技术
...
**问题**: 出现 "hackCheck version = 3" 警告，所有漏洞利用失败
**适用设备**: BE3600 2.5G (RD15)、BE6500 Pro 等新款设备
**解决方案**: 
- ✅ **自动处理**: 工具已内置 hackCheck=3 绕过技术
- 使用多种字符过滤绕过方法：Octal编码、Base64编码、字符替换
- 如仍然失败，可能需要：
  ```bash
  # 1. 尝试降级固件到较早版本
  # 2. 使用硬件调试接口 (UART/JTAG)
  # 3. 等待新的漏洞发现
  ```

**技术说明**: hackCheck=3 过滤字符: `= [ \n \` ; | $ &`
工具使用三种绕过技术：
- **Printf八进制编码**: `printf "cmd\073more" | sh` 
- **Base64编码**: `echo Y21kO21vcmU= | base64 -d | sh`
- **字符替换**: `echo "cmdSmire" | tr "S" ";" | sh`
